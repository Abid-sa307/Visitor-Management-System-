...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit Security Question</h1>
    ...
    <div class="card shadow">
        <div class="card-body">
            <form method="POST" action="{{ route('security-questions.update', $securityQuestion) }}">
                @csrf
                @method('PUT')
                            ...
                            @if(auth()->user()->role === 'superadmin')
                                <select name="company_id" id="companySelect" class="form-select" required>
                                    ...
                                    @foreach($companies as $company)
                                        <option value="{{ $company->id }}" {{ $securityQuestion->company_id == $company->id ? 'selected' : '' }}>
                                            {{ $company->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="branch_id" id="branchSelect" class="form-select">
                                ...
                                @foreach($branches as $branch)
                                    <option value="{{ $branch->id }}" {{ $securityQuestion->branch_id == $branch->id ? 'selected' : '' }}>
                                        {{ $branch->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="question" class="form-control" value="{{ old('question', $securityQuestion->question) }}" required>
                    @error('question')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="type" id="typeSelect" class="form-select" required>
                        ...
                        <option value="yes_no" {{ old('type', $securityQuestion->type) == 'yes_no' ? 'selected' : '' }}>Yes/No</option>
                        <option value="multiple_choice" {{ old('type', $securityQuestion->type) == 'multiple_choice' ? 'selected' : '' }}>Multiple Choice</option>
                    </select>
                    @error('type')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3 {{ $securityQuestion->type !== 'multiple_choice' ? 'd-none' : '' }}" id="optionsDiv">
                    <label class="form-label">Options (one per line)</label>
                    <textarea name="options_text" id="optionsText" class="form-control" rows="4" placeholder="Option 1&#10;Option 2&#10;Option 3">{{ $securityQuestion->options ? implode("\n", $securityQuestion->options) : '' }}</textarea>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-check mb-3">
                            <input type="checkbox" name="is_required" id="isRequired" class="form-check-input" value="1" {{ old('is_required', $securityQuestion->is_required) ? 'checked' : '' }}>
                            <label class="form-check-label" for="isRequired">Required Question</label>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-check mb-3">
                            <input type="checkbox" name="is_active" id="isActive" class="form-check-input" value="1" {{ old('is_active', $securityQuestion->is_active) ? 'checked' : '' }}>
