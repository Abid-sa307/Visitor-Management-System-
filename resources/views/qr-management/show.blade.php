...
function downloadQRCode() {
    ...
    img.onload = function() {
        ...
        @if($branch)
        ctx.fillText('Scan to Check-in at {{ addslashes($branch->name) }}', width / 2, instructionY);
        @else
        ...
        const urlText = '{{ $branch ? route('qr.scan', ['company' => $company, 'branch' => $branch]) : route('qr.scan', $company) }}';
