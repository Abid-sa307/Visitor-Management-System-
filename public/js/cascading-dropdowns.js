...
class CascadingDropdowns {
    constructor() {
        ...
        this.departmentButton = document.querySelector(
    ...
    initializeForCompanyUser() {
        ...
        if (branchOptions && Object.keys(branches).length > 0) {
            ...
            let selectedBranches = urlParams.getAll("branch_id[]");
            if (selectedBranches.length === 0) {
                for (const [key, value] of urlParams.entries()) {
                    if (key.match(/^branch_id\[\d*\]$/)) {
    ...
    filterDepartmentsByBranches(branchIds) {
        ...
        const urlParams = new URLSearchParams(window.location.search);
        let selectedDepartments = urlParams.getAll("department_id[]");
        if (selectedDepartments.length === 0) {
            for (const [key, value] of urlParams.entries()) {
                if (key.match(/^department_id\[\d*\]$/)) {
        ...
        if (!hasMatchingDepartments) {
            departmentOptions.innerHTML =
                '<div class="text-muted">No departments for selected branch(es)</div>';
        }
    ...
    lockDropdown(type) {
        const button =
            type === "branch" ? this.branchButton : this.departmentButton;
        if (button) {
    ...
    clearDropdown(type) {
        const options = document.getElementById(
            type === "branch" ? "branchOptions" : "departmentOptions",
        );
        const text = document.getElementById(
            type === "branch" ? "branchText" : "departmentText",
        );

        if (options) options.innerHTML = "";
        if (text)
            text.textContent =
                type === "branch" ? "All Branches" : "All Departments";
    }
    ...
    loadBranches(companyId) {
        ...
        // Handle indexed array parameters (e.g. branch_id[0]=1)
        if (selectedBranches.length === 0) {
            for (const [key, value] of urlParams.entries()) {
                if (key.match(/^branch_id\[\d*\]$/)) {
                    selectedBranches.push(value);
            ...
            .then((data) => {
                ...
                if (branches.length > 0) {
                    branches.forEach((branch) => {
                        const isChecked = selectedBranches.includes(
                            String(branch.id),
                        );
                        const div = document.createElement("div");
                        div.className = "form-check";
                    ...
                    branchOptions.innerHTML =
                        '<div class="text-muted">No branches available</div>';
                }
            })
            .catch(() => {
                branchOptions.innerHTML =
                    '<div class="text-muted">Error loading branches</div>';
            });
    }
    ...
    loadDepartments(companyId) {
        ...
        if (branchCheckboxes.length > 0) {
            ...
            branchCheckboxes.forEach((cb) =>
                params.append("branch_id[]", cb.value),
            );
            url += `?${params.toString()}`;
        }
        ...
        fetch(url, {
            ...
            .then((data) => {
                ...
                if (Array.isArray(data)) {
                    ...
                    departments = Object.entries(data || {}).map(
                        ([key, val]) => {
                            if (typeof val === "object" && val !== null)
                                return { id: key, ...val };
                            return { id: key, name: val };
                ...
                if (departments.length > 0) {
                    departments.forEach((dept) => {
                        const isChecked = selectedDepartments.includes(
                            String(dept.id),
                        );
                        const div = document.createElement("div");
                        div.className = "form-check";
                    ...
                    if (window.updateDepartmentText)
                        window.updateDepartmentText();
                } else {
                    departmentOptions.innerHTML =
                        '<div class="text-muted">No departments available</div>';
                }
            })
            .catch(() => {
                departmentOptions.innerHTML =
                    '<div class="text-muted">Error loading departments</div>';
            });
    }
}
